...
# def plot_results1
def plot_CS(N, M, f, Results_Dict, plot_CS_width=False, 
                    palette=None, save_fig=False, fig_info_dict={}):
    ...
    if not plot_CS_width:
        ax.plot(NN, m_star*np.ones(NN.shape), '--', label=r'$m^*$', color='k')
...
def plot_hists(N, StoppingTimesDict, palette=None, plot_mean_val=False, 
    ...
    for i, key in enumerate(StoppingTimesDict):
        ...
        st = ST.mean()
        plt.hist(ST, label=key, alpha=opacity, color=color, 
                    density=True) 
        if plot_mean_val:
...
def CompareMethodsOneTrial(M, f, S, methods_dict,  nG=100, save_fig=False, fig_info_dict={}, 
    ...
    methods_dict        :dict keys are strings denoting method names, 
                                and values are dicts that represent the 
                                additional keyword arguments to be sent 
                                into the function 'run_one_expt'
    fig_info_dict       :dict either empty or contains the following keys 
                            'figname', 'title', 'xlabel', 'ylabel'
    """
    N = len(M)
    Pi = M / M.sum()
    ...
    if plot_results: 
        plot_CS(N, M, f, Results_Dict=Results_Dict, plot_CS_width=plot_CS_width, 
                    save_fig=save_fig, fig_info_dict=fig_info_dict)
    if return_vals: 
        return Results_Dict


def getStoppingTimesDistribution(methods_dict, N, N1, a=0.5,
    ...
    for trial in range_:
        ...
        M, f, S = generate_MFS(N_vals=(N1, N-N1), N=N, 
                                M_ranges = M_ranges,
                                f_ranges = f_ranges, a = a)
        ...
        for key in Results_Dict:
            ...
            W = U-L 
            st = first_threshold_crossing(W, th=epsilon, max_time=N, upward=False)
