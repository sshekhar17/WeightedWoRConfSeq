...
def get_sampling_distribution2(unseen, M, f, S, method_name='propMS',
                                samp_func=None, 
                                samp_kwargs=None):
    """
    ...
    if (samp_func is  None): 
        ...
        if method_name =='propMS':
            ...
            raise Exception(f'{method_name} is not a valid sampling-method name')
...
def get_range_of_next_payoff(m, unseen, q_t, mu_0, M, S, method_name='propMS', 
...
def get_range_of_next_bet(min_payoff, max_payoff, lambda_max=2, tol=1e-10):
    ...
    # sanity check 
    assert min(lower*min_payoff, lower*max_payoff,
                upper*min_payoff, upper*max_payoff) > -1 
    return lower, upper 


def get_next_CV_bet(cv_vals, payoff_vals, beta_max=0.5, tol=1e-10, 
                        smoothing_term=1.0):
    """
...
def one_step_update(grid, wealth, seen, unseen, M, f, S, Payoff_vals, 
                                cv_vals, beta_vals, method_name='propMS',
                                samp_func=None, samp_kwargs=None, 
                                use_CV=False, beta_max=0.5,
                                f_over_S_range=None, lambda_max=2.5, 
                                alpha=0.05):
    unseen = unseen.astype(int) 
    Pi = M/np.sum(M)
    Pi_ = Pi[unseen]
    ...
    q_t = get_sampling_distribution2(unseen, M, f, S, method_name=method_name,
                                samp_func=samp_func, samp_kwargs=samp_kwargs)
    ...
    for i, m in enumerate(grid):
        ...
        min_payoff, max_payoff = get_range_of_next_payoff(m, unseen,
                                            q_t, mu_0, M, S,
                                            method_name=method_name, 
                                            f_over_S_range=f_over_S_range,
                                            use_CV=use_CV, beta=beta)
        # step 4: get the range of allowed lambda values
        l_min, l_max = get_range_of_next_bet(min_payoff, max_payoff,
        ...
        if (lambda_ * val <= -1): 
            ...
            print(f'lambda = {lambda_:.2f}, and payoff = {val:.2f}, product = {lambda_*val:.2f}')
    ...
    Payoff_vals = np.concatenate((Payoff_vals, current_payoffs.reshape((-1,1))), axis=1)
...
def run_one_expt(M, f,  S, method_name='propMS', lambda_max=2.5,
                    beta_max=0.5, nG=100, use_CV=False, 
                    f_over_S_range=None, alpha=0.05, logical_CS=False, 
                    intersect=False, return_payoff=False):
    N = len(M)                    
    ...
    Error_flag = False
    for t in range(N):
        result = one_step_update(grid=grid, wealth=wealth, seen=seen,
                                    unseen=unseen, M=M, f=f, S=S, 
                                    Payoff_vals=Payoff_vals, cv_vals=cv_vals,
                                    beta_vals=beta_vals,method_name=method_name,
    ...
    if logical_CS or intersect:
        LowerCS, UpperCS = predictive_correction1(LowerCS, UpperCS, 
                                            ...
                                            Pi = M/M.sum(), f=f, intersect=intersect,
                                            logical=logical_CS)
    if return_payoff:
...
def main(A=0.1, use_CV=False):
    ...
    nG = 100
    lambda_max = 2
    result_propMS = run_one_expt(M, f,  S, method_name='propMS', lambda_max=lambda_max,
                    beta_max=0.5, nG=nG, use_CV=use_CV, 
                    f_over_S_range=f_over_S_range, alpha=0.05, logical_CS=False, 
                    intersect=False, return_payoff=False)
    grid, _, L1, U1, _, _ = result_propMS
    
    result_propM = run_one_expt(M, f,  S, method_name='propM', lambda_max=lambda_max,
                    beta_max=0.5, nG=nG, use_CV=use_CV, 
                    f_over_S_range=None, alpha=0.05, logical_CS=False, 
                    intersect=False, return_payoff=False)
    _, _, L2, U2, _, _ = result_propM
    
    result_unif = run_one_expt(M, f,  S, method_name='uniform', lambda_max=lambda_max,
                    beta_max=0.5, nG=nG, use_CV=use_CV, 
                    f_over_S_range=None, alpha=0.05, logical_CS=False, 
                    intersect=False, return_payoff=False)
...
def testCV(A=0.1):
    ...
    nG = 100
    lambda_max = 2
    result_propMS = run_one_expt(M, f,  S, method_name='propM', lambda_max=lambda_max,
    ...
    result_propM = run_one_expt(M, f,  S, method_name='propM', lambda_max=lambda_max,
                    beta_max=0.5, nG=nG, use_CV=True, 
                    f_over_S_range=None, alpha=0.05, logical_CS=False, 
                    intersect=False, return_payoff=False)
    _, _, L2, U2, _, _ = result_propM
